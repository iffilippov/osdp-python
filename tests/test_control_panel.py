...
class ControlPanelTestCase(unittest.TestCase):
    ...
    def test_cp_checksum_unsecure(self):
        conn = SerialPortOsdpConnection(
            port="/dev/tty.wchusbserial1420", baud_rate=9600
        )
        cp = ControlPanel()
        ...
        cp.add_device(
            connection_id=bus_id, address=0x7F, use_crc=False, use_secure_channel=False
        )
        ...
        result = cp.reader_led_control(
            connection_id=bus_id,
            address=0x7F,
            reader_led_controls=ReaderLedControls(granted_led),
        )
        print("\r\n")
        print(result)
        ...
        result = cp.reader_led_control(
            connection_id=bus_id,
            address=0x7F,
            reader_led_controls=ReaderLedControls(denied_led),
        )
        print("\r\n")
        print(result)
