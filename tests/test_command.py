...
class CommandTestCase(unittest.TestCase):
    ...
    def test_reader_led_control_command_granted_checksum(self):
        ...
        command = ReaderLedControlCommand(
            address=0x7F, reader_led_controls=ReaderLedControls(reader_led_controls)
        )
        content = command.build_command(device)
        self.assertEqual(
            content.hex().upper(), "537F15000269000002020102000A0000000000009D"
        )
    ...
    def test_reader_led_control_command_denied_checksum(self):
        ...
        content = command.build_command(device)
        self.assertEqual(
            content.hex().upper(), "537F15000169000002020101000A0000000000009F"
        )
    ...
    def test_reader_buzzer_control_command_checksum(self):
        ...
        command = ReaderBuzzerControlCommand(
            address=0x7F, reader_buzzer_control=reader_buzzer_control
        )
        content = command.build_command(device)
    ...
    def test_reader_text_output_command_checksum(self):
        ...
        command = ReaderTextOutputCommand(
            address=0x7F, reader_text_output=reader_text_output
        )
        content = command.build_command(device)
    ...
    def test_set_date_time_command_checksum(self):
        ...
        timestamp = datetime.datetime(
            year=2019, month=11, day=12, hour=9, minute=2, second=0
        )
    ...
    def test_reader_mfg_command_checksum(self):
        ...
        command = ManufacturerSpecificCommand(
            address=0x7F, manufacturer_data=manufacturer_data
        )
        content = command.build_command(device)
    ...
    def test_reader_led_control_command_granted_crc(self):
        ...
        content = command.build_command(device)
        self.assertEqual(
            content.hex().upper(), "537F16000569000002020102000A0000000000009134"
        )
    ...
    def test_reader_led_control_command_denied_crc(self):
        ...
        content = command.build_command(device)
        self.assertEqual(
            content.hex().upper(), "537F16000669000002020101000A00000000000098F1"
        )
    ...
    def test_reader_text_output_command_crc(self):
        ...
        self.assertEqual(
            content.hex().upper(),
            "537F1D00076B00010000000F68656C6C6F2C20756E6976657273650370",
        )
    ...
    def test_set_date_time_command_crc(self):
        ...
        timestamp = datetime.datetime(
            year=2019, month=0x0B, day=0x1D, hour=0x10, minute=0x11, second=0x12
        )
        command = SetDateTimeCommand(address=0x7F, timestamp=timestamp)
        content = command.build_command(device)
