...
class Nak:
    ...
    def __repr__(self):
        return "Error: {0}\n Data: {1}".format(
            self.error_code.name, self.extra_data.hex()
        )
...
class DeviceCapability:
    ...
    def parse_data(data: bytes):
        function = (
            CapabilityFunction(data[0]) if data[0] <= 14 else CapabilityFunction.Unknown
        )
        compliance = data[1]
        number_of = data[2]
    ...
    def __repr__(self):
        if (
            self.function == CapabilityFunction.ReceiveBufferSize
            or self.function == CapabilityFunction.LargestCombinedMessageSize
        ):
            return "  Function: {0}\n" "      Size: {1}".format(
                self.function.name,
                int.from_bytes(
                    bytes([self.compliance, self.number_of]), byteorder="little"
                ),
            )
        else:
            return (
                ...
                "Compliance: {1}\n"
                " Number Of: {2}".format(
                    self.function.name, self.compliance, self.number_of
                )
            )
...
class ReaderStatus:
    ...
    def __repr__(self):
        return (
            "Reader Status: ["
            + ", ".join([str(status) for status in self.statuses])
            + "]"
        )
...
class OutputControl:
    ...
    def __init__(
        self, output_number: int, output_control_code: OutputControlCode, timer: int
    ):
    ...
    def build_data(self) -> bytes:
        ...
        return bytes(
            [
                self.output_number,
                self.output_control_code.value,
                timer_bytes[0],
                timer_bytes[1],
            ]
        )
...
class RawCardData:
    ...
    def __init__(
        self, reader_number: int, format_code: FormatCode, bit_count: int, data: bytes
    ):
        self.reader_number = reader_number
...
class KeypadData:
    ...
    def __repr__(self):
        return (
            ...
            "    Bit Count: {1}\n"
            "         Data: {2}".format(
                self.reader_number, self.bit_count, self.data.hex().upper()
            )
        )
