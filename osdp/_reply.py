...
class Reply(Message):
    ...
    def __init__(
        self, data: bytes, connection_id: UUID, issuing_command: Command, device: Device
    ):
        ...
        self._security_block_type = (
            (data[6] & 0xFF) if is_secure_control_block_present else 0
        )
        if is_secure_control_block_present:
            self._secure_block_data = data[(self.REPLY_MESSAGE_HEADER_SIZE + 2) :][
                : (secure_block_size - 2)
            ]
        else:
        ...
        if (
            self.security_block_type
            == SecurityBlockType.ReplyMessageWithDataSecurity.value
        ):
            self._extract_reply_data = self.decrypt_data(device)

        if is_using_crc:
            self._is_data_correct = self.calculate_crc(data[:-2]) == int.from_bytes(
                data[-2:], byteorder="little"
            )
        else:
            self._is_data_correct = self.calculate_checksum(
                data[:-1]
            ) == int.from_bytes(data[-1:], byteorder="little")
    ...
    @staticmethod
    def parse(
        data: bytes, connection_id: UUID, issuing_command: Command, device: Device
    ):
    ...
    def build_reply(self, address: int, control: Control) -> bytes:
        command_buffer = bytearray(
            [self.SOM, (self.address | 0x80), 0x00, 0x00, control.control_byte]
        )
    ...
    def __repr__(self):
        return "Connection ID: {0} Address: {1} Type: {2}".format(
            self._connection_id, self.address, self.type
        )
