...
class Device(object):
    ...
    def is_security_established(self) -> bool:
        return (
            self.message_control.has_security_control_block
            and self._secure_channel.is_established
        )

    @property
    def is_online(self) -> bool:
        return (
            self._last_valid_reply + datetime.timedelta(seconds=5)
    ...
    def get_next_command_data(self):
        ...
        if self._use_secure_channel and not self._secure_channel.is_initialized:
            return SecurityInitializationRequestCommand(
                self.address, self._secure_channel.server_random_number
            )

        if self._use_secure_channel and not self._secure_channel.is_established:
            return ServerCryptogramCommand(
                self.address, self._secure_channel.server_cryptogram
            )
    ...
    def initialize_secure_channel(self, reply):
        if (
            reply.security_block_type
            == SecurityBlockType.SecureConnectionSequenceStep2.value
        ):
            self._secure_channel.select_scbk(reply.secure_block_data[0])

        reply_data = reply.extract_reply_data
        self._secure_channel.initialize(
            reply_data[:8], reply_data[8:16], reply_data[16:32]
        )
