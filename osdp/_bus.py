...
class Bus:
    ...
    def idle_line_delay(self) -> timedelta:
        return timedelta(
            milliseconds=(1000.0 / self._connection.baud_rate * 16.0) * 100
        )
    ...
    def add_device(
        ...
        self._configured_devices[address] = Device(
            address, use_crc, use_secure_channel, master_key
        )
    ...
    def run_polling_loop(self):
        ...
        while not self._is_shutting_down:
            ...
            time_difference = timedelta(milliseconds=200) - (
            ...
            for device in list(self._configured_devices.values()):
                ...
                try:
                    ...
                    log.exception(
                        "Error while sending command %s and receiving reply", command
                    )
                    self._connection.close()
                    continue
    ...
    def send_command_and_receive_reply(
        self, data: bytearray, command: Command, device: Device
    ) -> Reply:
        command_data = None
        try:
        ...
        if not self.wait_for_rest_of_message(
            reply_buffer, self.extract_message_length(reply_buffer)
        ):
