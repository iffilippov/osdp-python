...
class ControlPanel:
    ...
    def id_report(self, connection_id: UUID, address: int) -> DeviceIdentification:
        return DeviceIdentification.parse_data(
            self.send_command(connection_id, IdReportCommand(address))
        )

    def device_capabilities(
        self, connection_id: UUID, address: int
    ) -> DeviceCapabilities:
        return DeviceCapabilities.parse_data(
            self.send_command(connection_id, DeviceCapabilitiesCommand(address))
        )

    def local_status(self, connection_id: UUID, address: int) -> LocalStatus:
        return LocalStatus.parse_data(
            self.send_command(connection_id, LocalStatusReportCommand(address))
        )

    def input_status(self, connection_id: UUID, address: int) -> InputStatus:
        return InputStatus.parse_data(
            self.send_command(connection_id, InputStatusReportCommand(address))
        )

    def output_status(self, connection_id: UUID, address: int) -> OutputStatus:
        return OutputStatus.parse_data(
            self.send_command(connection_id, OutputStatusReportCommand(address))
        )

    def reader_status(self, connection_id: UUID, address: int) -> ReaderStatus:
        return ReaderStatus.parse_data(
            self.send_command(connection_id, ReaderStatusReportCommand(address))
        )

    def output_control(
        self, connection_id: UUID, address: int, output_controls: OutputControls
    ) -> bool:
        reply = self.send_command(
            connection_id, OutputControlCommand(address, output_controls)
        )
        return reply.type == ReplyType.Ack or reply.type == ReplyType.OutputStatusReport

    def reader_led_control(
        self, connection_id: UUID, address: int, reader_led_controls: ReaderLedControls
    ) -> bool:
        reply = self.send_command(
            connection_id, ReaderLedControlCommand(address, reader_led_controls)
        )
    ...
    def add_device(
        self, connection_id: UUID, address: int, use_crc: bool, use_secure_channel: bool
    ):
        bus = self._buses.get(connection_id)
        if bus is not None:
    ...
    def on_reply_received(self, reply: Reply):
        ...
        if reply.type == ReplyType.Nak:
            ...
            self.on_local_status_report_reply_received(
                reply.address, LocalStatus.parse_data(reply)
            )

        elif reply.type == ReplyType.InputStatusReport:
            self.on_input_status_report_reply_received(
                reply.address, InputStatus.parse_data(reply)
            )

        elif reply.type == ReplyType.OutputStatusReport:
            self.on_output_status_report_reply_received(
                reply.address, OutputStatus.parse_data(reply)
            )

        elif reply.type == ReplyType.ReaderStatusReport:
            self.on_reader_status_report_reply_received(
                reply.address, ReaderStatus.parse_data(reply)
            )

        elif reply.type == ReplyType.FormattedReaderData:
            self.on_formatted_reader_data_reply_received(
                reply.address, reply.extract_reply_data
            )

        elif reply.type == ReplyType.RawReaderData:
            self.on_raw_card_data_reply_received(
                reply.address, RawCardData.parse_data(reply)
            )

        elif reply.type == ReplyType.KeypadData:
            self.on_keypad_data_reply_received(
                reply.address, KeypadData.parse_data(reply)
            )
    ...
    def on_local_status_report_reply_received(
        self, address: int, local_status: LocalStatus
    ):
        log.debug("%s < Local status received %s", address, local_status)

    def on_input_status_report_reply_received(
        self, address: int, input_status: InputStatus
    ):
        log.debug("%s < Input status received %s", address, input_status)

    def on_output_status_report_reply_received(
        self, address: int, output_status: OutputStatus
    ):
        log.debug("%s < Output status received %s", address, output_status)

    def on_reader_status_report_reply_received(
        self, address: int, reader_status: ReaderStatus
    ):
        log.debug("%s < Reader status received %s", address, reader_status)

    def on_formatted_reader_data_reply_received(
        self, address: int, formatted_reader_data: bytes
    ):
        log.debug(
            "%s < Formatted reader data received %s", address, formatted_reader_data
        )
